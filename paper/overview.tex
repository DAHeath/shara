...
systems~\cite{albarghouthi12a}, where the body of each has at most
one uninterpreted predicate.
%
...
$\cc{dbl}$ were called more than once in $\cc{dblsAbs}$ then multiple
copies of the body of the procedure would be inlined. And if the body
of this procedure were large, the inlining would quickly become
  ...
  \begin{floatrow}[2]
    \ffigbox[.45\textwidth] {%
      {\caption{$\mcchc$ as a directed hypergraph. Each relational
          ...
          represented by a hyperedge. Each hyperedge is labelled by the
...
Given an arbitrary recursion-free CHC system $S$, \sys~generates an
equivalent CDD system $S'$ and solves $S'$ directly.
%
In general, $S'$ may have size exponential in the size of
$S$.
%
However, \sys~generates equivalent systems via heuristics analogous to
